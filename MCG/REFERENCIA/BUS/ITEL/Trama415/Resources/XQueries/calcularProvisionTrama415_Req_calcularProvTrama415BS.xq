(:: pragma bea:global-element-parameter parameter="$input_Parameters" element="ns2:Input_Parameters" location="../Schemas/PS_ITEL_CALCULARPROVISIONTRAMA415.xsd" ::)
(:: pragma bea:global-element-return element="ns1:callFunctionStateless" location="../WSDLs/trama415Provision_DB.wsdl" ::)
...
declare namespace xf = "http://tempuri.org/ITEL/Trama415/Resources/XQueries/calcular/";
...
declare function xf:calcular($input_Parameters as element(ns2:Input_Parameters))
    as element(ns1:callFunctionStateless) {
        <ns1:callFunctionStateless name = "Trama415Provision_DF_CalcularProvTrama415"> 
            <ns1:parameterList>
                <ns0:TRAMA_EJECUCION>
                    ...
                    <ns0:PARAMETROSTRAMA>                        
                        <ns0:CODIGO_COBRO>{ data($input_Parameters/PARAMETROSTRAMA/PARAMETRO[NOMBRE='BONIFICAR']/VALOR) }</ns0:CODIGO_COBRO>
                    ...
                    <ns0:PARAMETROSNEGOCIO>
                        ...
                        <ns0:HOSTID_PAQUETE>{ data($input_Parameters/PARAMETROSNEGOCIO/PARAMETRO[NOMBRE='HOST_ID']/VALOR) }</ns0:HOSTID_PAQUETE>
                        <ns0:VALOR>{ data($input_Parameters/PARAMETROSNEGOCIO/PARAMETRO[NOMBRE='VALOR']/VALOR) }</ns0:VALOR>
                        <ns0:TIPO_PAQUETE>{ data($input_Parameters/PARAMETROSTRAMA/PARAMETRO[NOMBRE='MONTORECARGA']/VALOR) }</ns0:TIPO_PAQUETE>
                        <ns0:TIPONOTIFICACION>{ data($input_Parameters/PARAMETROSTRAMA/PARAMETRO[NOMBRE='ID_SM_BRMS']/VALOR) }</ns0:TIPONOTIFICACION>
                        <ns0:NOTIFICACION>{ data($input_Parameters/PARAMETROSNEGOCIO/PARAMETRO[NOMBRE='NOTIFICACION']/VALOR) }</ns0:NOTIFICACION>
                        <ns0:PAQUETES>
                        			<ns0:SIZE>{count($input_Parameters/PARAMETROSNEGOCIO/PARAMETRO[NOMBRE='OFFER_ID']/VALOR[string(.)])}</ns0:SIZE>
			                        <ns0:PAQUETE>
                                        <ns0:CODIGO_PAQUETE>{ data($input_Parameters/PARAMETROSNEGOCIO/PARAMETRO[NOMBRE='OFFER_ID']/VALOR) }</ns0:CODIGO_PAQUETE>
                                        <ns0:PLATAFORMA>{ data($input_Parameters/PARAMETROSNEGOCIO/PARAMETRO[NOMBRE='PLATAFORMA']/VALOR) }</ns0:PLATAFORMA>
                                    </ns0:PAQUETE>
                        </ns0:PAQUETES>
                    </ns0:PARAMETROSNEGOCIO>
