(:: pragma bea:global-element-parameter parameter="$input_Parameters1" element="ns0:Input_Parameters" location="../Schemas/PS_ITEL_VALIDARTRAMA406.xsd" ::)
(:: pragma bea:global-element-return element="ns0:WS_Result" location="../Schemas/PS_ITEL_VALIDARTRAMA406.xsd" ::)

declare namespace ns0 = "http://www.comcel.com.co/ValidarTrama406/";
declare namespace xf = "http://tempuri.org/ITEL/Trama406/Resources/XQueries/validarTrama406_Res/";
...
declare function xf:validarTrama406_Res($cadenaParametros as xs:string, $in as xs:integer)
    as element(ns0:WS_Result) {
        let $params := fn:tokenize($cadenaParametros, ';')
        return
            <ns0:WS_Result>
            	{if( count($params) != 6 or $params[1] = "" or  $params[2] = "" or  $params[3] = "" or  $params[4] = ""
            	    or string-length($params[1]) > 100 or string-length($params[2]) > 5 or string-length($params[3]) > 75 or string-length($params[6]) > 3 or string-length($params[5]) > 12) then
                      <CODIGO>912</CODIGO>
                  	 ...
                  	 <CODIGO>931</CODIGO>
                 else if( string($params[6]) = '' or string($params[5]) = '' ) then
                     <CODIGO>912</CODIGO>      	              	 
                ...
                <MENSAJE>
                    ...
                    <MONTOCREDITO>{$params[5]}</MONTOCREDITO>
...
xf:validarTrama406_Res($cadenaParametros,$in)