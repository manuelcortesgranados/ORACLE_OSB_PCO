...
(:: pragma bea:global-element-parameter parameter="$input_Parameters1" element="ns3:Input_Parameters" location="../Schemas/PS_ITEL_CALCULARPROVISIONTRAMA308.xsd" ::)
(:: pragma bea:global-element-return element="ns2:callFunctionStateless" location="../WSDLs/trama308Provision_DB.wsdl" ::)

declare namespace xf = "http://tempuri.org/ITEL/Trama308/Resources/XQueries/calcularProvisionTrama308_Req/";
declare namespace ns2 = "http://xmlns.oracle.com/Trama308Provision/Trama308Provision_DF_Trama308Provision";
declare namespace ns1 = "http://xmlns.oracle.com/bpel";
declare namespace ns3 = "http://www.comcel.com.co/CalcularProvisionTrama308/";
...
declare function xf:cargaElegidos($elegidos as xs:string)
 {
    	...
    	return <ns0:Detail>{$min}</ns0:Detail>
};

declare function xf:calcularProvisionTrama308_Req($input_Parameters1 as element(ns3:Input_Parameters))
    as element(ns2:callFunctionStateless) {
    	let $fechaExpiracion := $input_Parameters1/SUSCRIPTOR/*:BestFnF_ActivationEndTime
    	return
        <ns2:callFunctionStateless name="Trama308Provision_DF_Trama308Provision">
        <ns2:parameterList>
	        <ns0:TRAMA_EJECUCION>
	                ...
	                {if($input_Parameters1/CODIGOIN < 51) then	                  
	                  <ns0:MIN_ELEGIDOS>{ xf:cargaElegidos( data($input_Parameters1/MIN_ELEGIDOS) ) }</ns0:MIN_ELEGIDOS>
        			   ...
        			   $elegidos   := $params,
        			   $cantEleg := xs:integer(fn:count($elegidos) div 2),        			   
        			   $minElegidos := string-join(
		                    		    for $i in (($cantEleg + 1) to fn:count($elegidos)) 		                    		   
		                               return $elegidos[$i]
		                               , ";")
        			   return
	                 ...
	                 <ns0:MIN_ELEGIDOS_REEMPLAZAR>{ xf:cargaElegidos( data($input_Parameters1/MIN_ELEGIDOS_REEMPLAZAR) ) }</ns0:MIN_ELEGIDOS_REEMPLAZAR>
	                   ...
	                   <ns0:MIN_ELEGIDOS_REEMPLAZAR>{ xf:cargaElegidos($minElegidos)}</ns0:MIN_ELEGIDOS_REEMPLAZAR>
	                    ...
	                    <ns0:PARAMETROSNEGOCIO>
	                        ...
	                        <ns0:ELEGIDOS_MODIFICADOS>{
	                        		xf:cargaElegidos($input_Parameters1/PARAMETROSNEGOCIO/PARAMETRO[NOMBRE='ELEGIDOS_MODIFICADOS']/VALOR)
	                        }</ns0:ELEGIDOS_MODIFICADOS>
	                    </ns0:PARAMETROSNEGOCIO>
	                    }
	                    {if($input_Parameters1/CODIGOIN < 51) then
...
xf:calcularProvisionTrama308_Req($input_Parameters1)