...
declare function xf:consultarOfferPS_Req($min as xs:string, $offerFirst as xs:string)
    as element(ns0:Input_Parameters) {
        <ns0:Input_Parameters>
            ...
            <OFFERID_FIRST>{ data($offerFirst) }</OFFERID_FIRST>
        </ns0:Input_Parameters>
};

declare variable $min as xs:string external;
declare variable $offerFirst as xs:string external;

xf:consultarOfferPS_Req($min, $offerFirst)